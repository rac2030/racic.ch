...
The `/timeline` page renders a **full chronological, per-commit view** of every blog, project, and wiki entry (drafts excluded in production): each entry is expanded into one event per git commit by `buildTimelineEvents`/`expandGitCommits` in `src/lib/timeline.ts` (Jest-tested), sorted newest-first by commit date. The module also owns the key math — `timelineKey(collection, id)` → `collection/slug` (reusing `stripMdExtension`) — and the exclusion list `TIMELINE_EXCLUDED_KEYS = ['blog/building-this-site-with-ai']`: the continuously-updated "Building this site with AI" build-log post (26 commits) is dropped entirely so it can't take over the timeline. Articles with no git history fall back to a single "published" event using `effectiveUpdatedDate`.

The layout is a **git-graph** beside the change cards: each `.timeline-entry` is a flex row of a `.timeline-rail` (a per-commit node `.rail-dot` centered on a single vertical line and linking to the GitHub commit, and a `::after` horizontal connector running from the spine into the card) plus a `.card` panel. The vertical line is drawn ONCE on `ul#timeline-list::before` (a 2px bright sky-blue gradient `#bae6fd`→`#38bdf8`→`#0ea5e9` with a soft glow, `position:relative` list, `top`/`bottom: 1.4rem` inset) so it is ONE continuous line with no per-entry gaps — the per-rail `::before` spines were removed entirely (a regression guard asserts `rail::before` content is `none`). Reusing the 404 page's card style (`--text-secondary`/`--text-muted`/`--accent` variables), each card (now a `div`, not an anchor) shows the title as its own `timeline-card-title` link, a description, a section icon, the **last-changed date at top-right with a "LAST UPDATED" label**, and a commit-footer link (`.timeline-commit`, flush to the card bottom via negative margins). The connector **feeds into that footer**: commit rows get a `.timeline-rail--commit` class that re-anchors the rail node and the `::after` connector so BOTH their centers sit on the commit-message row (`.timeline-rail--commit` sets `--node-y: 1.4rem` = the footer content row; the dot uses `bottom: calc(var(--node-y) - 0.6rem)` and the connector `bottom: calc(var(--node-y) - 0.1rem)` because both elements carry `transform: translate(-50%,-50%)` whose half-height shifts the center up by 6px / 1px respectively — measured dot 695 ≈ connector 696 ≈ footer line 695). The footer itself starts with a `.timeline-commit-line` segment (2px, flush at the card's left edge) that continues the rail connector inside the card up to the `#shortHash` (first 7 hex chars) mono badge — the short hash doubles as the **label on that connecting line** — followed by the ellipsized commit message. The page header (title + muted subtitle) is a **reusable `src/components/PageHeader.astro` component** — props `title`, `subtitle?`, `icon?` (a FontAwesome class rendered before the title in accent color), `align?` (`'left'`|`'center'`); it does NOT set a font-size so the global `h1` (3em) applies. It is now used by **every** page header across the site so they all look identical: blog (`fa-pen-nib`), projects (`fa-microchip`), wiki (`fa-book`), bookmarks (`fa-bookmark`), about (`fa-user`), tags (`fa-tags`), tag (`fa-tag`), and timeline (`fa-clock-rotate-left`) — the icon styling that previously only appeared on the wiki/bookmarks headers now applies everywhere, and all are left-aligned (the `align`/``` `page-header-centered` centering class is available but unused, so the wiki/bookmarks headers dropped their old centered look to match blog/projects). This replaced the scattered per-page `<h1>`+`<p>` blocks (bookmarks/about/tags/tag used inline styles; the timeline header previously inherited `.page-404 h1`'s giant 6em and was matched to the standard 3em by dropping its `.page-404` wrapper for a `section.section > .container` layout). **Timeline mobile layout:** a `@media (max-width: 640px)` block shrinks the `.timeline-rail` width from `4.5rem`→`2.5rem`, reduces `.timeline-entry` gap (`0.75rem`→`0.5rem`) and the h3 `padding-right` reserved for the date badge (`9rem`→`5rem`) so the card gains usable width and the title isn't crushed at phone widths (no horizontal overflow; the card stays inside the viewport).

**Astro-scoping gotcha:** these commit overrides MUST be declared with higher specificity (`.timeline-entry .timeline-rail--commit::after` etc.) — Astro rewrites `.timeline-rail::after` to `.timeline-rail[data-astro-cid]:after`, giving base `::after` (0,2,1) the SAME specificity as the scoped `.timeline-rail--commit::after` (0,2,1), so source order decides and the base `top: 1.4rem` would otherwise win and pin the connector at the card top while the node moved to the footer. Also keep a single copy of the override block (ast edit duplication wrecked the cascade once). Scrolling uses the **browser scrollbar** (no inner scroll container): the first 10 entries render immediately and the rest are lazily revealed in batches of 8 as the visitor scrolls near viewport bottom (a `scroll`/`resize` listener with `requestAnimationFrame` checking `window.scrollY` against `scrollHeight`, `bottom` reached via a "Back to Home" `btn btn-primary` link).
...
**Semantic versioning & releases** — Before the build, `semantic-release` (configured in `.releaserc.json`) reads the commit history on `master` and, if there are release-worthy changes (`feat`, `fix`, `BREAKING CHANGE`, ...), it bumps `package.json`'s `version` by conventional-commit semantics (patch/minor/major), commits the bump, tags it, and creates a GitHub Release whose notes are generated from the grouped commit messages. The `build` job then compiles the site against the bumped version. The current version is rendered in the footer as a link (e.g. `v1.0.0`), read from `package.json` at build time in `Footer.astro`. Clicking it opens a **release-notes modal** instead of navigating away: the modal fetches `https://api.github.com/repos/rac2030/racic.ch/releases?per_page=5` at runtime (client-side) and lists the **past 5 releases**, each rendered as a version header linking to its GitHub release entry (`/releases/tag/vX.Y.Z`) plus the release body/notes. The dialog is a **holodeck-styled, draggable, resizable window**: it opens centered in the visible viewport at a fixed **680×480** default with the body scrolling internally (an earlier version measured the content height and clamped to `max-height: calc(100vh - 2rem)`, which on a tall enough page filled the whole viewport height and pushed the bottom/south resize handles off-screen so they couldn't be grabbed — the fixed default height solves that). `openModal()` also calls `fitDialog()` **synchronously before** the async GitHub fetch resolves, so the window is centered and correctly sized from the very first frame rather than flashing a mispositioned small box at the top-left until the release data arrives. It can be dragged by its header, and resized from 8 edge/corner handles (n/s/e/w/ne/nw/se/sw) — drag and resize clamp the window and its edges to stay within the viewport (never extending into non-visible parts that would require scrolling). Because the modal was originally rendered inside `<footer class="site-footer">`, and that footer has `backdrop-filter: blur(4px)`, browsers treat the footer as a containing block for the modal's `position: fixed`, anchoring the dialog to the footer instead of the browser window top. The modal is therefore teleported to `document.body` on init (`document.body.appendChild(modal)`) so no ancestor has `backdrop-filter`/`transform`/`filter`, and its `position: fixed` resolves against the viewport where it is centered. The release body is rendered as **markdown** client-side via the `marked` library (imported in the footer's bundled script) with `gfm: true, breaks: true`; the rendered content is left-aligned. The holodeck look adds cyan corner brackets, a subtle grid/scanline overlay, a top energy beam, and a "HOLODECK" header label. A "View all releases" link at the bottom jumps to the GitHub releases page. The modal is dismissed via the close button, the backdrop, or the Escape key. Because the release list changes on every release, it is fetched at runtime rather than baked in at build time; the GitHub API allows cross-origin requests, so no proxy is needed. The e2e suite (`tests/e2e/site.spec.ts`, in the `Release notes modal` block) intercepts the API call with a mock response and asserts the 5-releases listing, per-version GitHub links, the "View all releases" link, markdown rendering (heading, list, link, code), left alignment, viewport containment/centering, header dragging, se-handle resizing, and close/backdrop/Escape dismissal; those tests block the site's service worker (`test.use({ serviceWorkers: 'block' })`) so the intercepted API fetch is not re-issued by the SW.
...
| Tag filtering | Autocomplete input, frequency-weighted tag cloud, active pills, URL persistence, AND multi-select; on blog/projects/wiki the filter row stacks as a full-width category list on top with the full-width tag bar + cloud below it, so the tag cloud spans the entire content card |
...
| Semantic versioning | Conventional-commit analysis via semantic-release: auto-bumps package.json major/minor/patch, tags, and creates GitHub Release with notes; footer version link (e.g. `v1.0.0`) opens a holodeck-styled, centered (even while content loads), draggable & resizable release-notes modal listing the past 5 releases from the GitHub API (fetched `cache: 'no-store'` — never stale, re-fetched on every open and refreshed live on `NEW_VERSION`) with left-aligned markdown-rendered bodies (via `marked`), each linking to its release entry, plus a "View all releases" link; it's teleported to `body` so the footer's `backdrop-filter` can't trap it |
| Service worker (offline + live updates) | Passport-style lazy caching with a content-hashed `CACHE_NAME`, background revalidation with byte-diff version bump; `no-store` requests (GitHub API) bypass the cache entirely; on `NEW_VERSION` the current page auto-replaces its own content in place (`document.open/write/close`, once per version via a `sessionStorage` guard) while other-resource updates show the blue "New version available" banner, which a `sessionStorage` flag (`racic-ch-banner-pending`) keeps visible across reloads/navigations until the user clicks it |
...
3. **Update detected** — if the new response body differs from the cached version (byte-by-byte comparison), the SW increments a version counter in localStorage and posts a `NEW_VERSION` message to all clients. The message now includes the changed request **`url`** so clients can tell which resource updated.
4. **no-store requests bypass the cache entirely** — requests made with `cache: 'no-store'` (e.g. the release-notes modal's GitHub API fetch) are served straight from the network via `shouldBypassCache(request)`: no cache read, no stale copy, no cache write. This fixed the release-notes modal being "always behind" — previously the cache-first strategy cached the GitHub API response and then served that stale copy on every subsequent open.
5. **Live content replacement** — when a `NEW_VERSION` refers to the page that is currently open (normalized path matches `location.href` via the `normalizePath` helper in `sw.ts`), `Base.astro` auto-replaces the visible content instead of waiting for a click: it fetches `location.href` with `cache: 'no-store'` (SW serves the freshly cached/network content) and swaps the document in place with `document.open()/write()/close()`. A per-version `sessionStorage` guard (`racic-ch-reload-<version>`) ensures the swap happens at most once per version, and a failed fetch clears the guard so it can retry. Updates to other resources show the blue "New version available" banner, and `showUpdateBanner()` mirrors the banner's visibility into a `racic-ch-banner-pending` `sessionStorage` flag that `Base.astro` re-reads on script init — so the banner survives a reload, the in-place document swap, and later navigations until the user actually dismisses it by clicking (previously the banner was effectively never visible: a concurrent current-page swap wiped it, and nothing re-shown it afterwards).
6. **Banner click** — clicking the banner clears the pending flag and sends a `SKIP_WAITING` message to the SW, which calls `self.skipWaiting()`. A `controllerchange` event fires on the client, triggering a page reload to activate the new SW. Because the SW calls `self.skipWaiting()` on install there is usually no `waiting` state, so when `swReg.waiting` is null the click falls back to a plain `location.reload()` to pick up the fresh content.
7. **Offline with cache** — when the network is unavailable but a cached version exists, the cached page loads silently. No offline banner is shown.

The release-notes modal also drops its one-time "loaded" guard: every open re-fetches the GitHub API (`cache: 'no-store'`), and if a `NEW_VERSION` message arrives while the modal is open it refreshes the listed releases live.

**E2E tests** (`tests/e2e/service-worker.spec.ts` — 19 tests) verify: SW registration, script content (install/fetch/activate/message listeners, localStorage usage, version tracking, client notification, the `no-store` bypass branch and `url` field), update banner existence/visibility/text/toggle, periodic check behavior, page functionality after SW activation, live auto-replacement (current page swap-once + `sessionStorage` guard), non-current updates showing only the banner, the banner rendered visibly (bounding-box check), and the banner staying visible after a reload until dismissed. `tests/e2e/site.spec.ts` adds a "freshness on reopen" test proving the modal fetches the latest releases instead of stale cached data.
...
4. **Audio generation** — produces WAV narration segments using Voicebox TTS (Qwen CustomVoice, preset `Ryan`, `0.6B`) with Piper/espeak-ng as fallback
...
The skill also documents Piper voice alternatives (fallback-only usage) and instructs the AI to ask the user before switching voices.
...
Phase 16 proved the video pipeline, Phase 18 gave it a star feature, and Phase 17 packaged the workflow as a reusable skill. This phase produced the first **new** Short since the skill shipped — and it deliberately changed the weakest link in the old videos: the voice. Instead of Piper, the narration was generated by **Voicebox** (local, from source) using the **Qwen CustomVoice** engine's preset speaker **`Ryan`** at **0.6B** on CPU. The verdict from the human reviewer was unambiguous: **"I liked the new sound."** — the first time the synthetic narration was something they actively wanted to keep.

### Why Voicebox

The Phase 16 videos used Piper (`en_US-lessac-medium`), which met the French-name mangling and rhythm issues honestly documented there. Voicebox's `qwen_custom_voice` engine reads as a natural, present narrator, and its preset profiles need **zero reference audio** — no cloning, you just pick a speaker. On this 32-core CPU box, `0.6B` synthesized each 3-8s segment in roughly 2-10s after a one-time ~1.2GB model download. Speaker `Ryan` (dynamic male, English) was picked for its energetic, rhythmic delivery — ideal for the self-aware narration voice. The `youtube-short` skill now **defaults to exactly this voice** and records that it is human-endorsed, so future Shorts don't drift back to another TTS.

### The Script (approved, then reality-checked)

Eight lines following the skill's hook → setup → deep dive → punchline → outro structure, narrating the wiki graph end to end: the floating constellation, circle color = category, the live filter collapse (`?category=security` → world-writable-files + scanning-filesystem-weak-restrictions), teal bookmark diamonds that stay only while a connected article is visible, double-click-to-navigate, the 5-second physics settle ("simulating gravity forever? That would melt my laptop."), and the self-reroast punchline — "the tests caught a real bug... the test was right. I was very wrong."

**The pacing problem:** raw, the 8 segments ran **71.7s** — comfortably over the 60s Short budget. Rather than rewriting the script, the audio was post-processed:

1. `silenceremove` trimmed ~0.44s of lead-in/lead-out silence per segment (both ends),
2. `atempo=1.15` sped each segment up, keeping the vocal rhythm intact,
3. the 8 processed segments now total **56.62s**, leaving a ~3s tail.

That "trim silence, then speed lightly (≤ ~1.2×)" trick, whenever a script runs long, is now written into the skill.

### Capturing the Graph (Playwright changed the rules)

The capture followed the Phase 16 driver but hit a real API change: **Playwright 1.62 removed `page.screencast()`**, so recording moved to `browser.newContext({ recordVideo })`, which yields one 1080×1920 VP8 `.webm` per page. Recorded webm files can stop earlier than the in-page clock suggests, so every shot was padded using **node-side `Date.now()`** wall-clock and recorded with a generous buffer, then trimmed to the narration window at assembly.

The wiki graph needed frame-level targeting, so an `addInitScript` wraps `canvas.getContext('2d')` and snapshots — on every `restore()` — the circles (wiki nodes), diamonds (bookmark nodes), and `fillText` titles of the current frame. A test hook, `RESOLVE_ME(title, kind)`, resolves a node title to exact viewport coordinates, which let the driver hover the two hovered nodes, double-click the ant node (verified navigation to `/wiki/ant/`), and place the "!! BUG !!" overlay ring at the right pixel. Debugging turned up a genuine bug in the tracker itself: it classified any 4-point shape as a diamond but had filtered `lineTo` points out of the candidate list, leaving 3 points — fixed by counting all path points (`moveTo` + `lineTo`).

### Assembly and Verification

FFmpeg trimmed each recording to its narration duration, `concat`-copied the segments into one silent video, concatenated the 8 processed WAVs into `audio_all.wav`, and muxed audio over video using **`-t <video-duration>` instead of `-shortest`** — `-shortest` combined with an infinite `apad` filter graph hangs the muxer forever (a painful, time-boxed find).

**Result:** `/tmp/shorts/final_short.mp4` — **59.52s, 1080×1920, H.264 25fps, AAC**, narration audible (mean −19.8 dB on `volumedetect`). Because the video can't be *watched* by the AI that made it, verification was pixel-and-luma based again: per-second frame extraction with luminance checks confirmed no blank frames on the dark `#05070c` background, plus the live tooltip and navigation assertions from the wiki-graph e2e layer.

### Known Issues

- The final ~3s after narration ends show the settled graph in silence — acceptable pacing, but a future pass could loop an outro sting to fill it.
- Subtitles were **deliberately skipped** this round to keep the graph fully visible; Phase 16's ASS-subtitle path still exists if captions are wanted.
...
**List/static pages**: the `.container` gains the `content-card` class (`padding: 2rem` override; `.container.content-width` variants like about keep their 780px max-width). Applied to blog, projects, wiki, bookmarks, tags index + tag page, timeline, about, π, 💩. Home (`/`) and the search page already presented their content inside carded regions (`.hero-content`, `.recent-list`, `.search-page-*`) and were left as-is. The tag list now spans the whole card too: `.wiki-filter-row` (blog/projects/wiki) is a column stack (`flex-direction: column`, `order` puts `.category-filter` first), so the category buttons sit on their own full-width row with the `.tag-filter` bar + open `.tag-filter-cloud` stretching the full content-card width below them. The tag cloud is **frequency-weighted**: callers pass a `counts` map (`Record<tag, articleCount>`), and `<TagFilter>` renders each `.tag-cloud-tag` with an inline `font-size` between `0.8rem` and `1.6rem`, linearly scaled between the least- and most-used tag (`0.8 + ((count − minN) / span) * 0.8`), so popular tags render visibly larger.
