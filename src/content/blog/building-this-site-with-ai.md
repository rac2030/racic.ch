...
| Tag filtering | Autocomplete input, tag cloud, active pills, URL persistence, AND multi-select; on blog/projects/wiki the filter row stacks as a full-width category list on top with the full-width tag bar + cloud below it, so the tag cloud spans the entire content card |
