...
5. **Live content replacement** — when a `NEW_VERSION` refers to the page that is currently open (normalized path matches `location.href` via the `normalizePath` helper in `sw.ts`), `Base.astro` auto-replaces the visible content instead of waiting for a click: it fetches `location.href` with `cache: 'no-store'` (SW serves the freshly cached/network content) and swaps the document in place with `document.open()/write()/close()`. A per-version `sessionStorage` guard (`racic-ch-reload-<version>`) ensures the swap happens at most once per version, and a failed fetch clears the guard so it can retry. Updates to other resources still show the blue banner.
6. **Banner click** — clicking the banner sends a `SKIP_WAITING` message to the SW, which calls `self.skipWaiting()`. A `controllerchange` event fires on the client, triggering a page reload to activate the new SW.
...
**E2E tests** (`tests/e2e/service-worker.spec.ts` — 17 tests) verify: SW registration, script content (install/fetch/activate/message listeners, localStorage usage, version tracking, client notification, the `no-store` bypass branch and `url` field), update banner existence/visibility/text/toggle, periodic check behavior, page functionality after SW activation, live auto-replacement (current page swap-once + `sessionStorage` guard) and non-current updates showing only the banner. `tests/e2e/site.spec.ts` adds a "freshness on reopen" test proving the modal fetches the latest releases instead of stale cached data.
