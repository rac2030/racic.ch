...
| Walkthrough | `stories/<id>/output/walkthrough.mp4` | 1920×1080, 4:00–5:30, **4 audio streams** (en default + de/fr/de-CH) |
...
npm run walkthrough:narration -- <id>                     # Voicebox TTS, paced per segment × 4 langs
npm run walkthrough:capture -- <id>                       # Playwright screen-records each segment at 1920×1080
npm run walkthrough:assemble -- <id>                      # trim/concat + mux 4 language-tagged audio tracks
npm run walkthrough:verify -- <id>                        # QA, then copy the file to video-output/
```

Story signal flow: `story.json` (segments, 4-language texts, scene actions, `coveredFeatures`, voicebox profiles) → `narration/<lang>/` wavs + `durations.json` (per-segment max across langs) → `segments/*.webm` + `captured.json` → `output/walkthrough.mp4`.

**Gotchas (documented from real failures — do not "simplify" blindly):**
- **MP4 language tags must be ISO-639-2/B 3-letter codes** (`eng`/`ger`/`fra`). The `assemble.mjs` `LANG_2_TO_3` map converts `en`→`eng`, `de`/`de-CH`→`ger`, `fr`→`fra`; `verify.mjs` applies the same mapping when matching tags. Using 2-letter tags silently drops all but the video stream's tag.
...
| ID | Type | Title | Date |
|----|------|-------|------|
| 6196a34e-d642-487a-936d-4e2bf8e69546 | 🔵 discovery | Unit test suite passes with full coverage | 2026-09-16 |
| dc5413c6-2c3b-4d98-8e59-5071ed0c853d | 🟣 feature | Frequency-weighted tag cloud implementation | 2026-09-16 |

**Key concepts:** testing, jest, unit-tests, code-quality, tag-cloud, frequency-weighting, user-interface
...
**Key concepts:** tag-cloud, frequency-weighting, user-interface, testing

### src/content/
| ID | Type | Title | Date |
|----|------|-------|------|
| 34290213-17e1-48a5-9519-f031648ae1a5 | 🔵 discovery | Wiki content inventory: 11 entries across 8 categories | 2026-09-14 |

**Key concepts:** wiki-content-inventory, tag-taxonomy, category-coverage, content-diversity

### src/pages/wiki/
| ID | Type | Title | Date |
|----|------|-------|------|
| 9edd0928-9e1a-4718-acb0-bcf5ae652a44 | 🟣 feature | Wiki index with graph visualization and filtering | 2026-09-13 |

**Key concepts:** force-directed-graph, category-filtering, tag-filtering, double-click-navigation, hover-tooltips
